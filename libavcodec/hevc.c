...
#if EncryptSAOPars
void swap_array(int * offset_abs, unsigned int *key){
    int u, v, dx, dy, z1, z2, z3, i, j;
    int offset_abs_p[4]; 
    u  = (*key)&0x00000001;
    *key = (*key) >> 1;
    v  = (*key)&0x00000001;
    ...
    *key = (*key) >> 1;
    ...
    i=0;j=0;
    z3=v*i+dy;
    offset_abs_p[((i+u*j+z1)&1)* 2 + ((z3+z2*j)&1)] = offset_abs[0];
    i=0;j=1;
    z3=v*i+dy;
    offset_abs_p[((i+u*j+z1)&1)*2+((z3+z2*j)&1)] = offset_abs[1];
    i=1;j=0;
    z3=v*i+dy;
    offset_abs_p[((i+u*j+z1)&1)*2+((z3+z2*j)&1)] = offset_abs[2];
    i=1;j=1;
    z3=v*i+dy;
    offset_abs_p[((i+u*j+z1)&1)*2+((z3+z2*j)&1)] = offset_abs[3];

    offset_abs[0] = offset_abs_p[0];
    offset_abs[1] = offset_abs_p[1];
    offset_abs[2] = offset_abs_p[2];
    offset_abs[3] = offset_abs_p[3];
}
static void hls_sao_param(HEVCContext *s, int rx, int ry)
{
    ...
    for (c_idx = 0; c_idx < 3; c_idx++) {
        ...
        if(sao->type_idx[c_idx] == SAO_EDGE && !sao_merge_left_flag && !sao_merge_up_flag) {
          /*  for (i = 0; i < 4; i++)
                printf("%d  ", sao->offset_abs[c_idx][i]);*/
            swap_array( sao->offset_abs[c_idx], &s->keybits );
          /*  for (i = 0; i < 4; i++)
                printf("%d  ", sao->offset_abs[c_idx][i]);
                        printf("%d \n", c_idx);*/
        }
