...
#if H_MV
static int getRefLayerPicIdc(int j,HEVCContext *s)
{
	int refLayerPicIdc = -1;
	int curj = 0,i;
	for(i =0;i<s->vps->m_numDirectRefLayers[s->nuh_layer_id];i++){
		if(curj==j){
			refLayerPicIdc = i;
			break;
		}
...
static int getNumActiveRefLayerPics(HEVCContext *s)
{
	int numBits = 1;
		while ((1 << numBits) < s->vps->m_numDirectRefLayers[s->nuh_layer_id]) {
			numBits++;
		}
	if(s->nuh_layer_id == 0 || numBits == 0)
		s->sh.active_num_ILR_ref_idx = 0;
	else if(s->vps->m_ilpSshSignalingEnabledFlag)
		s->sh.active_num_ILR_ref_idx = numBits;
	else if(!s->sh.inter_layer_pred_enabled_flag)
		s->sh.active_num_ILR_ref_idx = 0;
	else if(s->vps->max_one_active_ref_layer_flag || s->vps->m_numDirectRefLayers[s->nuh_layer_id] ==1)
		s->sh.active_num_ILR_ref_idx = 1;
	else
		s->sh.active_num_ILR_ref_idx = s->sh.m_numInterLayerRefPicsMinus1 + 1;
	...
	if (!sh->dependent_slice_segment_flag) {
...
#if POC_RESET_FLAG
		{
			...
			if(s->pps->num_extra_slice_header_bits > iBits){
				sh->m_discardableFlag = get_bits1(gb);
				print_cabac("discardable_flag",sh->m_discardableFlag);
				iBits++;
			}
		...
		if (s->sps->separate_colour_plane_flag) {
			...
			print_cabac("colour_plane_id", sh->pic_output_flag);
		}
#endif
		print_cabac("nal_unit_type", s->nal_unit_type);
...
#if H_MV
			print_cabac("slice_pic_order_cnt_lsb",sh->pic_order_cnt_lsb);
#else
...
#else
		unsigned int interLayerPredIdcPresentFlag = 0;

		if(s->nuh_layer_id > 0 && NumILRRefIdx>0 && !s->vps->m_ilpSshSignalingEnabledFlag)
		{
			s->sh.inter_layer_pred_enabled_flag = get_bits1(gb);
			print_cabac("inter_layer_pred_enabled_flag", s->sh.inter_layer_pred_enabled_flag);
			if (s->sh.inter_layer_pred_enabled_flag && NumILRRefIdx >1) {
				int numBits = 1;
				while ((1 << numBits) < NumILRRefIdx) {
					numBits++;
				}
				if(!s->vps->max_one_active_ref_layer_flag){
					s->sh.m_numInterLayerRefPicsMinus1 = get_bits(gb,numBits) + 1;
					print_cabac("num_inter_layer_ref_pics_minus1", s->sh.m_numInterLayerRefPicsMinus1);
				}
				if(getNumActiveRefLayerPics(s) != NumILRRefIdx){
					interLayerPredIdcPresentFlag = 1;
					for(i=0;i<s->sh.active_num_ILR_ref_idx;i++){
						s->sh.inter_layer_pred_layer_idc[i] =get_bits(gb,numBits);
						print_cabac("inter_layer_pred_layer_idc",s->sh.inter_layer_pred_layer_idc[i]);
					}
		...
		if(!interLayerPredIdcPresentFlag){
			for(i=0; i<NumILRRefIdx;i++){
				s->sh.inter_layer_pred_layer_idc[i] = getRefLayerPicIdc(i, s);
			}
		}

		getNumActiveRefLayerPics(s);

#endif
#else
		if (s->nuh_layer_id > 0)
			s->sh.active_num_ILR_ref_idx = s->vps->m_numDirectRefLayers[s->nuh_layer_id];
#endif
#endif
...
#else
	if (s->pps->slice_header_extension_present_flag) {
		unsigned int posFollSliceSegHeaderExtLen;
		sh->m_sliceSegmentHeaderExtensionLength = get_ue_golomb_long(gb);
		print_cabac("slice_segment_header_extension_length",sh->m_sliceSegmentHeaderExtensionLength);
		posFollSliceSegHeaderExtLen = get_bits_count(gb);

		sh->m_poc_ResetIdc = 0;
		if(s->pps->m_pocResetInfoPresentFlag){
			sh->m_poc_ResetIdc = get_bits(gb,2);
			print_cabac("poc_reset_idc",sh->m_poc_ResetIdc);
		}

		sh->m_pocResetPeriodId = 0;
		if(sh->m_poc_ResetIdc != 0){
			sh->m_pocResetPeriodId = get_bits(gb,6);
			print_cabac("poc_reset_period_id",sh->m_pocResetPeriodId);
		}

		if(sh->m_poc_ResetIdc == 3){
			sh->m_fullPocResetFlag = get_bits1(gb);
			print_cabac("full_poc_reset_flag",sh->m_fullPocResetFlag);

			sh->m_pocLsbVal = get_bits(gb,s->sps->log2_max_poc_lsb);
			print_cabac("poc_lsb_val",sh->m_pocLsbVal);
		}

		sh->m_pocMsbValRequiredFlag = (s->nal_unit_type == NAL_CRA_NUT) ||  (s->nal_unit_type == NAL_BLA_N_LP) ||
				(s->nal_unit_type == NAL_BLA_W_RADL) || (s->nal_unit_type == NAL_BLA_W_LP);

		sh->m_pocMsbValPresentFlag = 1;
		if(!sh->m_pocMsbValRequiredFlag){
			sh->m_pocMsbValPresentFlag = get_bits1(gb);
			print_cabac("poc_msb_val_present_flag",sh->m_pocMsbValPresentFlag);
		}

		if(sh->m_pocMsbValPresentFlag){
			sh->m_pocMsbVal = get_ue_golomb_long(gb);
			print_cabac("poc_msb_val",sh->m_pocMsbVal);
		}

		while((get_bits_count(gb)-posFollSliceSegHeaderExtLen)<(sh->m_sliceSegmentHeaderExtensionLength * 8)){
			get_bits1(gb);//slice_segment_header_extension_data_bit
		}
...
#define POS(c_idx, x, y)                                                              \
	...
	if (SAMPLE_CTB(s->skip_flag, x_cb, y_cb)) { //ICI
...
static int hls_coding_unit(HEVCContext *s, int x0, int y0, int log2_cb_size)
{
	...
	if (SAMPLE_CTB(s->skip_flag, x_cb, y_cb)) {//ICI, appel de la méthode de prediction
