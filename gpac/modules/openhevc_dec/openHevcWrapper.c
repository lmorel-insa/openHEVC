...
#else
OpenHevc_Handle libOpenShevcInit(int nb_pthreads_wpp, int nb_pthreads_frame, int thread_type, int nb_pthreads_wpp1, int nb_pthreads_frame1, int thread_type1) {
    /* register all the codecs */
    int i;
    OpenHevcWrapperContexts *openHevcContexts = av_mallocz(sizeof(OpenHevcWrapperContexts));
    OpenHevcWrapperContext  *openHevcContext;
    ...
    for(i=0; i < openHevcContexts->nb_decoders; i++){
        if(i==1) {
            nb_pthreads_wpp = nb_pthreads_wpp1;
            nb_pthreads_frame = nb_pthreads_frame1;
            thread_type = thread_type1;
        }
        openHevcContext = openHevcContexts->wraper[i] = av_malloc(sizeof(OpenHevcWrapperContext));
        av_init_packet(&openHevcContext->avpkt);
        openHevcContext->codec = avcodec_find_decoder(AV_CODEC_ID_HEVC);
        ...
        av_opt_set_int(openHevcContext->c, "threads", nb_pthreads_wpp, 0);
        av_opt_set_int(openHevcContext->c, "threads_frame", nb_pthreads_frame, 0);
